...
class BaseTrainer:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self._log_path = os.path.join(self.args.log_path, self.args.label, name, 'log/')
        util.create_directories_dir(self._log_path)

        if hasattr(args, 'save_path'):
            self._save_path = os.path.join(self.args.log_path, self.args.label, name, 'save/')
    ...
    def _save_best(self, bert_config, model: torch.nn.Module, tokenizer: PreTrainedTokenizer, optimizer: Optimizer,
                   value: float, iteration: int, label: str, extra=None):
        if value > self._best_results[label]:
            print()
            self._logger.info("[%s] Best model in iteration %s: %s%% f1" % (label, iteration, value))
            self._save_model(bert_config, self._save_path, model, tokenizer, iteration,
                             optimizer=optimizer if self.args.save_optimizer else None,
                             save_as_best=True, name='model_%s' % label, extra=extra)
            self._best_results[label] = value

    def _save_model(self, bert_config: AlbertConfig, save_path: str, model: torch.nn.Module,
        ...
        save_file_path = os.path.join(dir_path, '%s.pth' % name)
        # FINETUNED_BERT_ENCODER_PATH = os.path.join(dir_path, 'model.bin')
        ...
        # save bert config
        bert_config.save_pretrained(dir_path)
