...
# from Edge_detection import RobertEdgeDetection
...
class CoPredictor(nn.Module):
    def __init__(self, cls_num, hid_size, biaffine_size, channels, ffnn_hid_size, dropout=0):
        ...
        self.mlp_rel_coll = MLP(hid_size, ffnn_hid_size, dropout=dropout)
...
class Model(nn.Module):
    def __init__(self, config,tokenizer):
        ...
        # self.bert = AlbertModel.from_pretrained(config.bert_name, cache_dir="./cache/", output_hidden_states=True)
        ...
        self.inp = conv_input_size
        self.out = config.conv_hid_size
        num_filters = config.lstm_hid_size
        self.dropout = nn.Dropout(config.emb_dropout)

        self.cln = LayerNorm(config.lstm_hid_size, config.lstm_hid_size, conditional=True)

        self.linear = nn.Linear(num_filters, config.label_num)
        self.linear2 = nn.Linear(2, self.lstm_hid_size)

        self.mlp1 = MLP(n_in=num_filters, n_out=num_filters, dropout=0.33)
        self.mlp2 = MLP(n_in=num_filters, n_out=num_filters, dropout=0.33)
        self.biaffine = Biaffine(n_in=num_filters, n_out=config.label_num, bias_x=True, bias_y=True)
    ...
    # def forward(self, bert_inputs,  pieces2word, sent_length):
        ...
        logit= self.linear(word_reps[:,0,:])
        h = self.dropout(self.mlp1(word_reps))
        t = self.dropout(self.mlp2(word_reps))
        outputs = self.biaffine(h, t)
        length = outputs.shape[1]
        maxpool = nn.MaxPool2d(length, stride=2)
        outs = maxpool(outputs.permute(0, 3, 1, 2).contiguous())
        out = torch.squeeze(outs)
        return logit+out
